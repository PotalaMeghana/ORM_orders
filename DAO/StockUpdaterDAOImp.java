...
public class StockUpdaterDAOImp implements StockUpdaterDAO {
	...
	{
		return jdbcTemplate.queryForObject("select prod_stock from slam_productstock where  prod_id=?", new Object[]{ productId}, Integer.class);
	}
    ...
    public void updateStocks(int prod_id, int qty) {
        ...
        try {
			...
			/*
			 * String sql = "SELECT * FROM slam_productstock FOR UPDATE";
			 * jdbcTemplate.query(sql, rs -> { String updateSql =
			 * "UPDATE slam_productstock SET prod_stock = prod_stock - ? WHERE prod_id = ?";
